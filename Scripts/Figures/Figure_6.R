library ("mdatools")
try (setwd ("C:/Users/ar123/dropbox/Kamila/Dane_uporzadkowane/UV"))   # AR dom
try (setwd ("C:/Users/ar/dropbox/Kamila/Dane_uporzadkowane/UV"))      # AR praca
...
mm=simcam(list(m.bs,m.ls,m.os,m.s,m.ts))
summary(mm)
res = predict(mm, Xt, ct)
#layout(1)
#plotPredictions(mm$calres, main = 'Training set 1',xticks = c(200),xticklabels = c("A"),
...
#
####################################################
idx=idx2    #   Tu jest wybor training setu !!
###################################################
#
po_obrobce_dfx=przed_obrobka              # Raw
#po_obrobce_dfx = po_obrobce_snv          # tylko SNV
...
res = predict(mm, Xt, ct)
#layout(1)
plotPredictions(mm$calres, main = 'Training set 2',xticks = c(200),xticklabels = c("A"),
                lab.cex=cex_labels,lab.col='black')
plotPredictions(mm$calres, main = "",xticks = c(200),xticklabels = c("A"),
                lab.cex=cex_labels,lab.col='black') 
xl <- (bquote('Objects'))   # Bez bolda
...
res = predict(mm, Xt, ct)
#layout(1)
plotPredictions(mm$calres, main = 'Training set 3',xticks = c(200),xticklabels = c("A"),
                lab.cex=cex_labels,lab.col='black')
plotPredictions(mm$calres, main = "",xticks = c(200),xticklabels = c("A"),
                lab.cex=cex_labels,lab.col='black') 
xl <- (bquote('Objects'))   # Bez bolda
